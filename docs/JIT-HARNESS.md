...
This package expresses **Codebase Inspection** as a typed, task-adaptive agent harness. Its concrete capability remains: Measure a codebase’s size, languages, and composition without changing it. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Codebase Inspection |
| --- | --- | --- |
| Memory | `layered_working_context` | Retain repository tree plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `layered_specification_plan` | Transform language detector into the ordered, bounded stages defined by this skill. |
...
| Action | `inspect_change_test_handoff` | Advance the workflow through pygount counters while preserving stop conditions and user authority. |
